  ...
  return loading ? (
    ...
    <div className="p-4 sm:p-6 lg:p-8 w-full max-w-7xl mx-auto">
      <div className="rounded-xl border border-gray-700 overflow-x-auto">
        <table className="min-w-187.5 w-full divide-y divide-gray-700 bg-slate-900 text-white text-sm sm:text-base">
          <thead className="bg-slate-800 uppercase sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 sm:px-6 py-3 text-left font-semibold">LTP</th>
              ...
              <th className="px-4 sm:px-6 py-3 text-left font-semibold">Low</th>

              <th className="px-4 sm:px-6 py-3 text-left font-semibold">Qty</th>

              {/* Sticky checkbox column */}
              <th className="px-4 sm:px-6 py-3 sticky right-0 bg-slate-800"></th>
            </tr>
          </thead>

          <tbody className="divide-y divide-gray-700">
            {filteredData.map((scrip, index) => {
              const percent_change = Number(scrip.percent_change);
              return (
                <tr key={index} className="hover:bg-slate-800">
                  ...
                  <td className="px-4 sm:px-6 py-4">{scrip.ltp}</td>

                  <td
                    className={`px-4 sm:px-6 py-4 text-center ${percent_change >= 0 ? "text-green-400" : "text-rose-400"}`}
                  >
                    {percent_change}
                  </td>

                  <td className="px-4 sm:px-6 py-4">{scrip.high}</td>

                  <td className="px-4 sm:px-6 py-4">{scrip.low}</td>

                  <td className="px-4 sm:px-6 py-4">{scrip.qty}</td>

                  {/* Sticky checkbox */}
                  <td className="px-4 sm:px-6 py-4 sticky right-0 bg-slate-900">
                    <input
                      type="checkbox"
                      name="select"
